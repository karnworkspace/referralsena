...
echo "False";
	...
	$qv = "select * from ag_agent order by id desc";
...
while($row = mysqli_fetch_array($query))
    $rows[] = $row;
foreach($rows as $row){ 
    	$idcard = substr_replace(stripslashes($row['idcard']),'xxxxx',5,-1);

	$fname = $row['fname'];
	$lname = $row['lname'];
	$phone = substr_replace(stripslashes($row['phone']),'xxxxx',5,-2);
	$email = substr_replace(stripslashes($row['email']),'xxxxx',5-2);
	$regisdate = $row['regis_date'];
	
    //$code = stripcslashes($row['agentcode']);
    echo "idcard = [".$idcard."]"."----".$fname."--".$lname."-----".$phone."------".$email."---------[ <span style='background:#47caa3;'>".$regisdate." </span>]<br />";     
