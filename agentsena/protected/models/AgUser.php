...
class AgUser extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, bud, role, username, password, project, active', 'required'),
			array('id', 'numerical', 'integerOnly'=>true),
			array('bud, role', 'length', 'max'=>5),
			array('name', 'length', 'max'=>50),
			array('username', 'length', 'max'=>60),
			array('password', 'length', 'max'=>15),
			array('project', 'length', 'max'=>400),
			array('active', 'length', 'max'=>2),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, bud, role, name, username, password, project, active', 'safe', 'on'=>'search'),
		);
	}
