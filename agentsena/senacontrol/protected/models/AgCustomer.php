...
class AgCustomer extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('update_by, create_by, id', 'numerical', 'integerOnly'=>true),
			array('agent_id', 'length', 'max'=>20),
			array('is_duplicate, sena_approve', 'length', 'max'=>1),
			...
			array('id, agent_id, fname, lname, project, budget, phone, idcard,comment,is_duplicate,create_date,active,create_by,user_ip,update_date,update_by,duplicate_lead_id,sena_approve', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search_cus()
	{
		...
		$array_project_id = json_decode(Yii::app()->user->project_view);
		$project_id = array();
		if($array_project_id){
	...
	public function search()
	{
		...
		$criteria->compare('phone',$this->phone,true);
		$criteria->compare('idcard',$this->idcard,true);
		$criteria->order = "sena_approve ASC,create_date DESC";
